...
const AddEditProcess = React.lazy(() => import("./add-edit-process"));
...
const TableHeaderFormat = (props) => {
  ...
  return [
    ...
    { field: 'processDtl', headerName: 'Process Detail', width: 200 },
...
export default function Process() {
  ...
  React.useEffect(() => {
    if(selectedRows.length === 1){
      const selected = processList.find(item => item._id === selectedRows[0])
    ...
    <section className="inspection-entry-form">
        ...
        <Drawer anchor={"right"} open={open} onClose={() => handleClose()} PaperProps={{ style: { width: 600 } }}> 
          <Suspense fallback={<Loader />}>
            <AddEditProcess process={selectedProcess} onClose={handleClose} /> 
          </Suspense>
        </Drawer>
 
        <Breadcrumbs aria-label="breadcrumb" style={{marginBottom: "1rem"}}>
          <Link underline="hover" color="inherit" href="/"> Masters </Link>
          <Typography sx={{ color: 'text.primary' }}>Process</Typography>
        </Breadcrumbs>

        <Typography className="title" color="primary">
            <AccountTree color="primary" style={{ fontSize: "3rem", margin: "-10px 0" }} /> Process
        </Typography>
        ...
        <div className="button-container">
            ...
            <IconButton onClick={() => deleteProcess()}> <DeleteIcon color={!!selectedRows.length ? "error" : "disabled"} /> </IconButton>
        ...
        <Card>
            <CardContent style={{ padding: "0px" }}>
                <DataGrid sx={DataGridStyle} rows={processList} columns={TableHeaderFormat({currentPage, pageSize})} getRowId={row => row._id}
                  ...
                  onRowSelectionModelChange={event => {
                    ...
                    const ids = processList.map((row) => row._id);
